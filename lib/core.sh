#!/bin/bash
# Autonomous Evolution Cycle - 公共库 (完整修复版)
# 提供所有shell脚本共享的函数
# 修复了所有语法兼容性问题，确保在所有bash环境中正常工作
...
aec_uuidgen() {
    if command -v uuidgen &> /dev/null; then
        ...
        # 兼容性备用方案
        echo "$(date +%s)-$$-$(dd if=/dev/urandom bs=4 count=1 2>/dev/null | od -An -tx1 | tr -d ' \n')"
    fi
}
...
time_get_current_slot() {
    ...
    # 检查各个时间槽（使用数值比较，完全兼容）
    if [[ $current_minutes -ge 300 && $current_minutes -le 420 ]]; then
        # 05:00-07:00 (300-420 minutes)
        echo "freeActivity"
    elif [[ $current_minutes -ge 420 && $current_minutes -le 480 ]]; then
        # 07:00-08:00 (420-480 minutes)
        echo "planning"
    elif [[ $current_minutes -ge 540 && $current_minutes -le 720 ]]; then
        # 09:00-12:00 (540-720 minutes)
        echo "deepWork"
    elif [[ $current_minutes -ge 840 && $current_minutes -le 1020 ]]; then
        # 14:00-17:00 (840-1020 minutes)
        echo "deepWork"
    elif [[ $current_minutes -ge 1260 && $current_minutes -le 1320 ]]; then
        # 21:00-22:00 (1260-1320 minutes)
        echo "consolidation"
    else
        echo "none"
    fi
...
heartbeat_check() {
    ...
    for task_file in "${AEC_WORKSPACE}/memory/working"/*.json; do
        ...
        status=$(jq -r '.status' "$task_file" 2>/dev/null || echo "error")
        progress=$(jq -r '.progress' "$task_file" 2>/dev/null || echo "0")
        started_at=$(jq -r '(.startedAt // .createdAt)' "$task_file" 2>/dev/null || echo "")
        
        if [[ "$status" == "in_progress" && "$progress" == "0" && -n "$started_at" ]]; then
            # 计算时间差（兼容版本）
            local current_epoch started_epoch diff_seconds
            
            # 使用GNU date或BSD date兼容方式
            ...
            if [[ "$started_epoch" != "0" ]]; then
                diff_seconds=$((current_epoch - started_epoch))
                local diff_minutes=$((diff_seconds / 60))
                
                # 30分钟阈值
                if [[ $diff_minutes -gt 30 ]]; then
                    log_warn "检测到零进度任务: $(jq -r '.title' "$task_file" 2>/dev/null)"
                    ((zero_progress_count++))
                    
                    # 检查是否启用自动激活
                    local auto_activate
                    auto_activate=$(config_load | jq -r '.enabledFeatures.autoTaskActivation // true' 2>/dev/null || echo "true")
