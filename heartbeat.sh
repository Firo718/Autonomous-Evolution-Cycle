#!/bin/bash
# Autonomous Evolution Cycle - Heartbeat集成 (修复版)
...
if [[ -f "${LIB_DIR}/core-fixed.sh" ]]; then
    source "${LIB_DIR}/core-fixed.sh"
else
...
perform_heartbeat_check() {
    ...
    echo "{
        \"timestamp\": \"$timestamp\",
        \"zeroProgressTasksCount\": ${#zero_progress_tasks[@]},
        \"tasksNeedingAttention\": $tasks_needing_attention,
        \"averageProgress\": $avg_progress,
        \"healthScore\": $health_score,
        \"status\": \"$(if [[ $health_score -ge 70 ]]; then echo "healthy"; elif [[ $health_score -ge 50 ]]; then echo "warning"; else echo "critical"; fi)\"
    }"
    ...
    echo "{\"timestamp\": \"$timestamp\", \"zeroProgressTasks\": ${#zero_progress_tasks[@]}, \"averageProgress\": $avg_progress, \"healthScore\": $health_score}" >> "$heartbeat_log" 2>/dev/null || true
